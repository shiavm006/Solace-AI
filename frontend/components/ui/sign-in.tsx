...
const GlassInputWrapper = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <div className={`rounded-xl border border-[var(--border)] bg-[var(--foreground)]/5 backdrop-blur-sm transition-colors focus-within:border-yellow-400/70 focus-within:bg-yellow-500/10 ${className || ''}`}>
    {children}
  </div>
);
      ...
      <section className={`flex-1 flex items-center justify-center p-6 md:p-8 ${!heroImageSrc ? 'w-full' : ''}`}>
        <div className={`w-full ${heroImageSrc ? 'max-w-md' : 'max-w-md'}`}>
          <div className="flex flex-col gap-6">
            ...
            <p className={`animate-element ${!isLogin ? 'animate-delay-800' : 'animate-delay-700'} text-center text-xs text-[var(--muted-foreground)] mt-2`}>
              {isLogin ? (
